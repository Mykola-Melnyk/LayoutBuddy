...
final class SpellCheckerService {
    private let spellDocTag: Int

    init() {
        self.spellDocTag = SpellCheckerService.performOnMain { NSSpellChecker.uniqueSpellDocumentTag() }
    }

    // Executes work on the main queue if we're currently on a background thread.
    private static func performOnMain<T>(_ work: @escaping () -> T) -> T {
