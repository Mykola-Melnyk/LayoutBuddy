...
final class AppCoordinator: NSObject {
    ...
    private var pendingWordProcessing: DispatchWorkItem?
#if DEBUG
    /// Captures text generated by `replaceLastWord` during tests.
    var testTypedText = ""
#endif
    ...
    private func processBufferedWordIfNeeded(keepFollowingBoundary: Bool = false) {
        ...
        pendingWordProcessing?.cancel()
        let item = DispatchWorkItem { [weak self] in
            guard let self else { return }
            self.pendingWordProcessing = nil
            guard !self.wordBuffer.isEmpty else { return }
            self.processBufferedWordNow(keepFollowingBoundary: keepFollowingBoundary)
        }
        pendingWordProcessing = item
        DispatchQueue.main.asyncAfter(deadline: .now() + 0.05, execute: item)
    }

    private func processBufferedWordNow(keepFollowingBoundary: Bool) {
        guard !wordBuffer.isEmpty else { return }
    ...
    private func sendBackspace(times: Int) {
        #if DEBUG
        // Simulate backspace by ignoring during tests
        #else
...
extension AppCoordinator {
    ...
    /// Trigger processing of the current word buffer in tests.
    ...
    /// Retrieve synthesized text produced during tests.
    func testGetTypedText() -> String { testTypedText }
}
#endif
